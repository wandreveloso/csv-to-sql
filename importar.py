...
# Função para limpar valores nulos
def clean_value(value):
    return value.strip() if value else None

# Função para formatar o valor para o comando SQL
...
# Leitura do cabeçalho do arquivo CSV
with open(csv_file_path, 'r', encoding='utf-8') as csv_file:
    reader = csv.reader(csv_file)
    header = next(reader)  # Ignora o cabeçalho

# Índices dos atributos no cabeçalho
...
# Listas para armazenar valores únicos
collections = set()
genders = set()
publishers = set()
persons = set()
...
with open('insert_commands.sql', 'w', encoding='utf-8') as sql_file:
    # Inserção nas tabelas intermediárias
    ...
    # Inserção na tabela principal (tb_book)
    with open(csv_file_path, 'r', encoding='utf-8') as csv_file:
        reader = csv.reader(csv_file)
        header = next(reader)  # Ignora o cabeçalho novamente
        ...
        for row in reader:
            ...
            # Verifica se a tabela tb_place (shelf) já foi inserida
            if place_shelf and place_shelf not in places:
                sql_file.write(f"INSERT INTO tb_place (shelf) VALUES ('{place_shelf}');\n")
                places.add(place_shelf)

            # Gera o comando INSERT INTO para a tabela principal (tb_book)
            sql_file.write(
            ...
            # Obtém os IDs correspondentes das tabelas intermediárias
            ...
            # Gera comandos INSERT INTO para tabelas intermediárias
            if collection_name:
                sql_file.write(
